...
async function login(){try{const pin=($('pin')?.value||'').trim(),username=($('username')?.value||'').trim().toLowerCase();let session=await ensureSession();currentUser=session.user;let result;if(loginMode==='admin'){if(!username)return toast('Masukkan username admin terlebih dahulu.');if(!/^\\d{4}$/.test(pin))return toast('PIN admin harus tepat 4 angka.');result=await sb.rpc('kiosk_login_pin',{p_username:username,p_pin:pin});}else{if(!username)return toast('Masukkan username kasir terlebih dahulu.');const reg=await sb.rpc('kiosk_register_cashier',{p_username:username});if(reg.error&&!/admin|dinonaktifkan|tidak menggunakan/i.test(reg.error.message||''))throw reg.error;result=await sb.rpc('kiosk_login',{p_username:username,p_password:null});}if(result.error&&/JWT|session|anonymous|not authenticated/i.test(result.error.message||'')){await sb.auth.signOut();session=await ensureSession();currentUser=session.user;if(loginMode==='admin')result=await sb.rpc('kiosk_login_pin',{p_username:username,p_pin:pin});else result=await sb.rpc('kiosk_login',{p_username:username,p_password:null});}if(result.error)throw result.error;const data=result.data;if(!data?.length)throw Error('Akun tidak ditemukan atau tidak aktif.');const account=data[0];if(loginMode==='admin'&&account.role!=='admin')throw Error('Akun ini bukan akun admin.');if(loginMode==='kasir'&&account.role==='admin')throw Error('Gunakan tombol Login Admin.');const prof=await sb.rpc('kiosk_profile',{p_username:account.username});if(prof.error)throw prof.error;if(!prof.data?.length)throw Error('Profil akun belum tersedia.');profile=prof.data[0];profile.auth_user_id=currentUser.id;if(!profile.aktif)throw Error('Akun tidak aktif.');localStorage.setItem('tokokasirherbal-username',profile.username);showApp();try{await loadAll()}catch(loadErr){console.error('loadAll after login',loadErr);toast('Login berhasil, tetapi data belum dapat dimuat: '+(loadErr?.message||'periksa data Supabase'));}}catch(e){console.error('login',e);const m=String(e?.message||e);if(/anonymous|disabled|sign.?in/i.test(m))toast('Login otomatis Supabase gagal. Aktifkan Anonymous Sign-Ins di Supabase Authentication.');else toast(m||'Login gagal')}}
