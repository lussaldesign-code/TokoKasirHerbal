...
function renderCatalogSettings(){const el=$('catalogSettingsList');if(!el)return;el.innerHTML=products.map(p=>{const on=dashboardCatalog.includes(p.id);const pid=escAttr(p.id);return '<label class="catalog-product-option '+(on?'selected':'')+'"><input type="checkbox" '+(on?'checked':'')+' data-catalog-id="'+pid+'"><img src="'+escAttr(p.gambar||'')+'" onerror="this.src=\\'https://placehold.co/80x60?text=Produk\\'"><span><b>'+esc(p.nama)+'</b><small>'+rp(p.harga_ecer)+' • Stok '+p.stok+'</small></span><em>'+((dashboardCatalog.indexOf(p.id)+1)||'')+'</em></label>'}).join('')||'<div class="note">Belum ada produk.</div>';el.querySelectorAll('input[data-catalog-id]').forEach(input=>input.addEventListener('change',()=>toggleDashboardCatalog(input.dataset.catalogId,input.checked)))}
...
function renderDashboardCatalog(){const dp=$('dashboardProducts');if(!dp)return;const list=dashboardCatalog.map(id=>products.find(p=>p.id===id)).filter(Boolean);dp.innerHTML=list.map(p=>'<div class="product" onclick="choosePrice(\\''+p.id+'\\')"><img src="'+escAttr(p.gambar||'')+'" onerror="this.src=\\'https://placehold.co/300x180?text=Herbal\\'"><div class="info"><b>'+esc(p.nama)+'</b><div class="price">'+rp(p.harga_ecer)+'</div><div class="stock">Sisa Stok: '+p.stok+'</div></div></div>').join('')||'<div class="note">Belum ada produk di katalog. Admin dapat mengaturnya dengan ikon ⚙️.</div>'}
