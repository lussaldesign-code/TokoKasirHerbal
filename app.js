...
function renderCatalogSettings(){const el=$('catalogSettingsList');if(!el)return;el.innerHTML=products.map((p,i)=>{const on=dashboardCatalog.includes(p.id);return '<label class="catalog-product-option '+(on?'selected':'')+'"><input type="checkbox" '+(on?'checked':'')+' onchange="toggleDashboardCatalog(\\''+escAttr(p.id)+'\\',this.checked)"><img src="'+escAttr(p.gambar||'')+'" onerror="this.src=\\'https://placehold.co/80x60?text=Produk\\'"><span><b>'+esc(p.nama)+'</b><small>'+rp(p.harga_ecer)+' • Stok '+p.stok+'</small></span><em>'+((dashboardCatalog.indexOf(p.id)+1)||'')+'</em></label>'}).join('')||'<div class="note">Belum ada produk.</div>'}
