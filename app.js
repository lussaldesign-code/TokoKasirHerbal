...
function renderReports(){
 ...
 const ds=sales.filter(s=>String(s.created_at).slice(0,10)===day), ms=sales.filter(s=>String(s.created_at).slice(0,7)===month);
 const income=ds.reduce((a,s)=>a+Number(s.total||0),0), mincome=ms.reduce((a,s)=>a+Number(s.total||0),0);
