...
function choosePrice(id){const p=products.find(x=>x.id===id);if(!p)return toast('Produk tidak ditemukan.');priceProduct=p;const btns=$('priceButtons');if(!btns)return;const prices=[['ecer','Harga Ecer',p.harga_ecer],['reseller','Harga Reseller',p.harga_reseller],['agen','Harga Agen',p.harga_agen],['grosir','Harga Grosir',p.harga_grosir]].filter(x=>Number(x[2])>0);$('priceName').textContent='Pilih Harga • '+p.nama;btns.innerHTML=prices.map(x=>'<button type="button" class="btn primary price-choice-btn" data-price-type="'+escAttr(x[0])+'">'+esc(x[1])+'<br><b>'+rp(x[2])+'</b></button>').join('');
btns.querySelectorAll('.price-choice-btn').forEach(btn=>btn.addEventListener('click',()=>addToCart(btn.dataset.priceType)));openModal('priceModal')}
