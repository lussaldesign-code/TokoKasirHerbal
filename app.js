...
let sb=null,currentUser=null,profile=null,products=[],agents=[],receivables=[],sales=[],saleItems=[],receivablePayments=[],users=[],purchases=[],cart=[],purchaseCart=[],priceProduct=null,selectedProductImage='';
...
async function loadAll(){const base=[sb.from('products').select('*').eq('aktif',true).order('nama'),sb.from('agents').select('*').eq('aktif',true).order('nama'),sb.from('receivables').select('*, agents(nama), sales(nomor_transaksi,created_at)').order('created_at',{ascending:false})];if(profile?.role==='admin'){base.push(sb.from('sales').select('*').order('created_at',{ascending:false}).limit(1000),sb.from('sale_items').select('*').order('created_at',{ascending:false}).limit(5000),sb.from('receivable_payments').select('*').order('created_at',{ascending:false}).limit(5000),sb.from('purchases').select('*').order('created_at',{ascending:false}).limit(500),sb.from('users').select('id,username,nama,role,aktif,created_at,updated_at,auth_user_id').order('nama'));}const results=await Promise.all(base);for(const x of results)if(x.error)throw x.error;const [p,a,r,s,si,rpms,pu,u]=results;products=p.data||[];agents=a.data||[];receivables=r.data||[];sales=profile?.role==='admin'?(s?.data||[]):[];saleItems=profile?.role==='admin'?(si?.data||[]):[];receivablePayments=profile?.role==='admin'?(rpms?.data||[]):[];purchases=profile?.role==='admin'?(pu?.data||[]):[];users=profile?.role==='admin'?(u?.data||[]):[];renderAll()}
...
function renderReports(){
 ...
 $('sales').innerHTML=ds.map(s=>{const ag=agents.find(a=>a.id===s.agent_id);return `<tr><td>${new Date(s.created_at).toLocaleTimeString('id-ID',{hour:'2-digit',minute:'2-digit'})}</td><td><b>${esc(s.nomor_transaksi)}</b></td><td>${esc(ag?.nama||'Umum')}</td><td>${rp(s.total)}</td><td>${rp(s.dibayar)}</td><td>${rp(Math.max(0,Number(s.total||0)-Number(s.dibayar||0)))}</td><td>${esc(reportPaymentLabel(s))}</td></tr>`}).join('')||'<tr><td colspan="7" class="note">Belum ada penjualan hari ini.</td></tr>';
