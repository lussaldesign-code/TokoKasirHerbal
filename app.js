...
function updateAccountView(){const w=$('accountWelcome'),em=$('accountEmail'),ei=$('accountEmailInput');if(w)w.textContent=(profile?.nama||'Akun')+' (@'+(profile?.username||'-')+')';if(em)em.textContent='Login: '+(profile?.username||'-');if(ei)ei.value='Username: '+(profile?.username||'-')}\nfunction defaultCatalogRacks(){return [{key:'Agarillus',label:'Produk Agarillus',icon:'🌿',visible:true},{key:'Herbal',label:'Produk Herbal Biasa',icon:'🌱',visible:true},{key:'Rak1',label:'Rak 1',icon:'📦',visible:true},{key:'Rak2',label:'Rak 2',icon:'📦',visible:true},{key:'Rak3',label:'Rak 3',icon:'📦',visible:true}]}\nasync function loadCatalogSettings(){try{const {data,error}=await sb.rpc('kiosk_get_catalog_settings');if(error)throw error;const saved=Array.isArray(data)?data:[];const base=defaultCatalogRacks();const existing=saved.filter(x=>x&&x.key&&x.label).map(x=>({...x,visible:x.visible!==false}));catalogRacks=existing.length?existing:base;const productKeys=new Set(products.map(p=>String(p.kategori||'')));productKeys.forEach(key=>{if(!catalogRacks.some(x=>x.key===key))catalogRacks.push({key,label:key,icon:'📦',visible:true})});if(!saved.length&&profile?.role==='admin'){await sb.rpc('kiosk_save_catalog_settings',{p_catalog:catalogRacks})}}catch(e){console.warn('loadCatalogSettings',e);catalogRacks=defaultCatalogRacks()}}\nfunction catalogRack(key){return catalogRacks.find(x=>x.key===key)||{key,label:key,icon:'📦',visible:true}}\nfunction syncCategoryControls(selected=''){const cats=catalogRacks.filter(x=>x.visible!==false);const filter=$('category');if(filter){const current=selected||filter.value;filter.innerHTML='<option value="Semua">Semua Katalog</option>'+cats.map(c=>'<option value="'+escAttr(c.key)+'">'+esc(c.icon||'📦')+' '+esc(c.label)+'</option>').join('');filter.value=cats.some(c=>c.key===current)?current:'Semua'}const rak=$('pKategori');if(rak){const current=selected||rak.value;rak.innerHTML=cats.map(c=>'<option value="'+escAttr(c.key)+'">'+esc(c.icon||'📦')+' '+esc(c.label)+'</option>').join('');if(current&&cats.some(c=>c.key===current))rak.value=current;else if(cats.length)rak.value=cats[0].key}}\nfunction renderCatalogSettings(){const el=$('catalogSettingsList');if(!el)return;el.innerHTML=catalogRacks.map((r,i)=>'<div class="catalog-setting-row"><div class="catalog-setting-icon">'+esc(r.icon||'📦')+'</div><div class="catalog-setting-info"><b>'+esc(r.label)+'</b><small>Kode: '+esc(r.key)+'</small></div><span class="catalog-order">'+(i+1)+'</span><button class="icon-btn" title="Edit katalog" onclick="editCatalogRack(\''+escAttr(r.key)+'\')">✏️</button><button class="icon-btn danger-icon" title="Hapus katalog" onclick="deleteCatalogRack(\''+escAttr(r.key)+'\')">🗑️</button></div>').join('')||'<div class="note">Belum ada katalog.</div>'}\nfunction openCatalogSettings(){if(profile?.role!=='admin')return toast('Hanya admin yang dapat mengatur katalog.');renderCatalogSettings();openModal('catalogModal')}\nasync function addCatalogRack(){const label=prompt('Nama katalog / rak baru:');if(!label?.trim())return;const name=label.trim();if(catalogRacks.some(x=>x.label.toLowerCase()===name.toLowerCase()))return toast('Nama katalog sudah ada.');const key='Rak_'+Date.now();catalogRacks.push({key,label:name,icon:'📦',visible:true});await saveCatalogSettings();renderCatalogSettings();syncCategoryControls();toast('Katalog '+name+' berhasil ditambahkan.')}\nasync function editCatalogRack(key){const r=catalogRack(key);const label=prompt('Nama katalog:',r.label);if(!label?.trim()||label.trim()===r.label)return;if(catalogRacks.some(x=>x.key!==key&&x.label.toLowerCase()===label.trim().toLowerCase()))return toast('Nama katalog sudah ada.');r.label=label.trim();await saveCatalogSettings();renderCatalogSettings();syncCategoryControls();renderProducts();toast('Nama katalog berhasil diubah.')}\nasync function deleteCatalogRack(key){const r=catalogRack(key);const count=products.filter(p=>p.kategori===key).length;if(count)return toast('Katalog ini masih berisi '+count+' produk. Pindahkan produknya terlebih dahulu.');if(catalogRacks.length<=1)return toast('Minimal satu katalog harus tersedia.');if(!confirm('Hapus katalog '+r.label+'?'))return;catalogRacks=catalogRacks.filter(x=>x.key!==key);await saveCatalogSettings();renderCatalogSettings();syncCategoryControls();toast('Katalog berhasil dihapus.')}\nasync function saveCatalogSettings(){const {error}=await sb.rpc('kiosk_save_catalog_settings',{p_catalog:catalogRacks});if(error)throw error}\n
