...
async function loadAll(){const base=[sb.from('products').select('*').eq('aktif',true).order('nama'),sb.from('agents').select('*').eq('aktif',true).order('nama'),sb.from('receivables').select('*, agents(nama), sales(nomor_transaksi,created_at)').order('created_at',{ascending:false})];if(profile?.role==='admin'){base.push(sb.from('sales').select('*').order('created_at',{ascending:false}).limit(1000),sb.from('sale_items').select('*').order('created_at',{ascending:false}).limit(5000),sb.from('receivable_payments').select('*').order('created_at',{ascending:false}).limit(5000),sb.from('purchases').select('*').order('created_at',{ascending:false}).limit(500),sb.from('purchase_items').select('*').order('created_at',{ascending:false}).limit(5000),sb.from('sale_returns').select('*').order('created_at',{ascending:false}).limit(500),sb.from('purchase_returns').select('*').order('created_at',{ascending:false}).limit(500),sb.from('users').select('id,username,nama,role,aktif,created_at,updated_at,auth_user_id').order('nama'));}const results=await Promise.all(base);for(const x of results)if(x.error)throw x.error;const [p,a,r,s,si,rpms,pu,pi,sr,pr,u]=results;products=p.data||[];agents=a.data||[];receivables=r.data||[];sales=profile?.role==='admin'?(s?.data||[]):[];saleItems=profile?.role==='admin'?(si?.data||[]):[];receivablePayments=profile?.role==='admin'?(rpms?.data||[]):[];purchases=profile?.role==='admin'?(pu?.data||[]):[];purchaseItems=profile?.role==='admin'?(pi?.data||[]):[];saleReturns=profile?.role==='admin'?(sr?.data||[]):[];purchaseReturns=profile?.role==='admin'?(pr?.data||[]):[];users=profile?.role==='admin'?(u?.data||[]):[];renderAll()}
