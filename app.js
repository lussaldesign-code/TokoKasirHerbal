...
let sb=null,currentUser=null,profile=null,products=[],agents=[],receivables=[],sales=[],saleItems=[],receivablePayments=[],users=[],purchases=[],purchaseItems=[],saleReturns=[],purchaseReturns=[],cart=[],purchaseCart=[],priceProduct=null,selectedProductImage='';
...
async function loadAll(){const required=[sb.from('products').select('*').eq('aktif',true).order('nama'),sb.from('agents').select('*').eq('aktif',true).order('nama'),sb.from('receivables').select('*, agents(nama), sales(nomor_transaksi,created_at)').order('created_at',{ascending:false})];const results=await Promise.all(required);for(const x of results)if(x.error)throw x.error;const [p,a,r]=results;products=p.data||[];agents=a.data||[];receivables=r.data||[];sales=[];saleItems=[];receivablePayments=[];purchases=[];purchaseItems=[];saleReturns=[];purchaseReturns=[];users=[];if(profile?.role==='admin'){const optional=[['sales',sb.from('sales').select('*').order('created_at',{ascending:false}).limit(1000)],['sale_items',sb.from('sale_items').select('*').order('created_at',{ascending:false}).limit(5000)],['receivable_payments',sb.from('receivable_payments').select('*').order('created_at',{ascending:false}).limit(5000)],['purchases',sb.from('purchases').select('*').order('created_at',{ascending:false}).limit(500)],['purchase_items',sb.from('purchase_items').select('*').order('created_at',{ascending:false}).limit(5000)],['sale_returns',sb.from('sale_returns').select('*').order('created_at',{ascending:false}).limit(500)],['purchase_returns',sb.from('purchase_returns').select('*').order('created_at',{ascending:false}).limit(500)],['users',sb.from('users').select('id,username,nama,role,aktif,created_at,updated_at,auth_user_id').order('nama')]];const optionalResults=await Promise.all(optional.map(async([name,q])=>{try{const x=await q;if(x.error){console.warn('loadAll '+name,x.error);return {data:[]}}return x}catch(e){console.warn('loadAll '+name,e);return {data:[]}}}));sales=optionalResults[0].data||[];saleItems=optionalResults[1].data||[];receivablePayments=optionalResults[2].data||[];purchases=optionalResults[3].data||[];purchaseItems=optionalResults[4].data||[];saleReturns=optionalResults[5].data||[];purchaseReturns=optionalResults[6].data||[];users=optionalResults[7].data||[]}renderAll()}
...
function openSaleReturn(id){ensureReturnModal();returnMode='sale';returnRefId=id;const items=saleItems.filter(x=>x.sale_id===id);$('returnTitle').textContent='Retur Barang Terjual';$('returnItems').innerHTML=items.map(x=>{const returned=saleReturns.filter(r=>r.id&&false).length;return '<div class="field" style="border:1px solid #e5ece7;padding:10px;border-radius:10px"><label>'+esc(x.nama_produk)+' — '+rp(x.harga)+'</label><div style="display:flex;gap:8px;align-items:center"><span class="note">Terjual '+x.qty+'</span><input id="retSale_'+x.id+'" type="number" min="0" max="'+x.qty+'" value="0" style="max-width:120px"></div></div>'}).join('');$('returnReason').value='';openModal('returnModal')}
function openPurchaseReturn(id){ensureReturnModal();returnMode='purchase';returnRefId=id;const items=purchaseItems.filter(x=>x.purchase_id===id);$('returnTitle').textContent='Retur Barang Dibeli';$('returnItems').innerHTML=items.map(x=>'<div class="field" style="border:1px solid #e5ece7;padding:10px;border-radius:10px"><label>Produk '+esc(products.find(p=>p.id===x.product_id)?.nama||x.product_id)+' — '+rp(x.harga_beli)+'</label><div style="display:flex;gap:8px;align-items:center"><span class="note">Dibeli '+x.qty+'</span><input id="retPur_'+x.id+'" type="number" min="0" max="'+x.qty+'" value="0" style="max-width:120px"></div></div>').join('');$('returnReason').value='';openModal('returnModal')}
