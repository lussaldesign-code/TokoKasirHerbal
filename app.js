...
async function checkForUpdate(){
  ...
  try{
    ...
    try{
      ...
      const api='https://api.github.com/repos/lussaldesign-code/TokoKasirLussal/releases/latest?t='+Date.now();
...
function syncAccountPinField(mode,prefix){const isAdmin=mode==='admin';const wrap=$(prefix+'PinWrap'),input=$(prefix+'UserPassword');if(wrap)wrap.classList.toggle('hidden',!isAdmin);if(input){input.value='';input.placeholder=isAdmin?'••••':'Kasir tidak menggunakan PIN';input.disabled=!isAdmin;input.setAttribute('aria-hidden',String(!isAdmin));}if(wrap){wrap.classList.toggle('pin-slide-in',isAdmin);}}
