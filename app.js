...
function renderCart(){const el=$('cart');if(!el)return;el.innerHTML=cart.length?cart.map((x,i)=>'<div class="cart-item"><div style="flex:1;min-width:0"><b>'+esc(x.nama)+'</b><small style="display:block;color:#789487">'+esc(x.type_label||'Ecer')+' • '+rp(x.harga)+'</small></div><div style="display:flex;align-items:center;gap:6px"><button class="btn secondary" style="padding:4px 8px" onclick="changeCartQty(11272,-1)">−</button><b>'+x.qty+'</b><button class="btn secondary" style="padding:4px 8px" onclick="changeCartQty(11272,1)">+</button><button class="btn secondary" style="padding:4px 8px" onclick="removeCartItem(11272)">✕</button></div><b style="min-width:85px;text-align:right">'+rp(x.harga*x.qty)+'</b></div>').join(''):'<div class="note">Belum ada produk di keranjang.</div>';const total=cart.reduce((n,x)=>n+Number(x.harga||0)*Number(x.qty||0),0);if($('total'))$('total').textContent=rp(total);changeAmount?.()}
...
function choosePrice(id){const p=products.find(x=>x.id===id);if(!p)return toast('Produk tidak ditemukan.');priceProduct=p;const btns=$('priceButtons');if(!btns)return;const prices=[['ecer','Harga Ecer',p.harga_ecer],['reseller','Harga Reseller',p.harga_reseller],['agen','Harga Agen',p.harga_agen],['grosir','Harga Grosir',p.harga_grosir]].filter(x=>Number(x[2])>0);$('priceName').textContent='Pilih Harga • '+p.nama;btns.innerHTML=prices.map(x=>'<button class="btn primary" onclick="addToCart('+JSON.stringify(x[0])+')">'+esc(x[1])+'<br><b>'+rp(x[2])+'</b></button>').join('');openModal('priceModal')}
...
function openUserEditor(id){if(profile?.role!=='admin')return toast('Hanya admin yang dapat mengelola akun.');const u=users.find(x=>x.id===id);if(!u)return toast('Akun tidak ditemukan.');$('editUserId').value=u.id;$('editUserNama').value=u.nama||'';$('editUserUsername').value=u.username||'';$('editUserRole').value=u.role||'karyawan';$('editUserAktif').value=u.aktif?'true':'false';$('editUserPassword').value='';$('editUserPassword').placeholder=u.role==='admin'?'Kosongkan jika PIN tidak diubah':'Kasir tidak menggunakan PIN';openModal('userModal')}
